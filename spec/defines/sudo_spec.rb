...
describe 'sudo::conf', :type => :define do
  ...
  describe 'when creating a sudo entry' do
    it do
      is_expected.to contain_sudo__conf('admins').with(:priority => params[:priority],
    ...
    it do
      is_expected.to contain_exec("sudo-syntax-check for file #{params[:sudo_config_dir]}#{params[:priority]}_#{title}").with('command' => "visudo -c -f '#{params[:sudo_config_dir]}#{params[:priority]}_#{title}' || ( rm -f '#{params[:sudo_config_dir]}#{params[:priority]}_#{title}' && exit 1)",
                                                                                                                              'refreshonly' => 'true')
    end
    ...
    it { is_expected.not_to contain_exec("sudo-syntax-check for file #{params[:sudo_config_dir]}#{params[:priority]}_#{title}").that_requires("File[#{filename}]") }
  ...
  describe 'when creating a sudo entry with single number priority' do
    ...
    it do
      is_expected.to contain_exec("sudo-syntax-check for file #{params[:sudo_config_dir]}0#{params[:priority]}_#{title}").with('command' => "visudo -c -f '#{params[:sudo_config_dir]}0#{params[:priority]}_#{title}' || ( rm -f '#{params[:sudo_config_dir]}0#{params[:priority]}_#{title}' && exit 1)",
                                                                                                                               'refreshonly' => 'true')
    end
